...
/**
 * Fake OAuth login for development (no real backend required)
 */
export async function fakeOAuthLogin(): Promise<void> {
  try {
    console.log("[Auth] Performing fake OAuth login...");

    // Create a fake user
    const fakeUser: User = {
      id: 12345,
      openId: "fake-oauth-user-" + Date.now(),
      name: "Demo User",
      email: "demo@semanticnoise.local",
      loginMethod: "oauth",
      lastSignedIn: new Date(),
    };

    // Save user info
    await setUserInfo(fakeUser);

    // Set a fake session token
    const fakeToken = "fake-session-token-" + Date.now();
    await setSessionToken(fakeToken);

    console.log("[Auth] Fake OAuth login successful", fakeUser);
  } catch (error) {
    console.error("[Auth] Fake OAuth login failed:", error);
    throw error;
  }
}
